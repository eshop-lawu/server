...
public class ShareFriendsEventHandle implements AsyncEventHandle<ShareFriendsEvent> {
    ...
    public void execute(ShareFriendsEvent event) {
        Result<RongYunDTO> shareUserDTOResult = memberService.getRongYunInfoByNum(event.getShareUserNum());
        ...
        if (StringUtils.isNotEmpty(event.getRoomHostNum()) || event.getAttendTypeEnum() != null || event.getMemberId() > 0) {
            ...
            param.setMemberId(event.getMemberId());
