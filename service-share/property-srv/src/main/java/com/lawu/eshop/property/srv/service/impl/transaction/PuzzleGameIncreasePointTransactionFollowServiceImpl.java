 ...
 */
@Service
@CompensatingTransactionFollow(topic = MqConstant.TOPIC_GAME_SRV, tags = MqConstant.TAG_PUZZLE_GAME_INCREASE_POINT)
public class PuzzleGameIncreasePointTransactionFollowServiceImpl
	...
	public void execute(PuzzleGameDeductionNotification puzzle) {
		...
		param.setPoint(String.valueOf(new BigDecimal(puzzle.getPoint()).intValue()));
		param.setBizId(String.valueOf(puzzle.getId()));
		param.setGmtExecute(new Date());
		param.setMemberTransactionTypeEnum(MemberTransactionTypeEnum.POINT_GAME_PUZZLE_IN);
